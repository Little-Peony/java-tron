...
/**
 * Unit tests for the ML-DSA-44 (0x19) verify precompile (FIPS 204 / Dilithium-2).
 * Input layout: [msg 32B | sig 2420B | pk 1312B]. Stateless — no chain DB.
 */
public class MlDsa44PrecompileTest {

  private static final DataWord MLDSA_ADDR = new DataWord(
  ...
  public void switchOff_returnsNull() {
    VMConfig.initAllowMlDsa44(0L);
    Assert.assertNull(PrecompiledContracts.getContractForAddress(MLDSA_ADDR));
  }

  @Test
  public void switchOn_returnsContract() {
    Assert.assertNotNull(PrecompiledContracts.getContractForAddress(MLDSA_ADDR));
  }
  ...
  public void validSignature_returnsOne() {
    ...
    PrecompiledContract pc = PrecompiledContracts.getContractForAddress(MLDSA_ADDR);
  ...
  public void tamperedMessage_returnsZero() {
    ...
    Pair<Boolean, byte[]> result =
        PrecompiledContracts.getContractForAddress(MLDSA_ADDR).execute(input);
  ...
  public void nullInput_returnsZero() {
    Pair<Boolean, byte[]> result =
        PrecompiledContracts.getContractForAddress(MLDSA_ADDR).execute(null);
  ...
  public void shortInput_returnsZero() {
    Pair<Boolean, byte[]> result =
        PrecompiledContracts.getContractForAddress(MLDSA_ADDR).execute(new byte[100]);
  ...
  @Test
  public void wrongLengthInput_returnsZero() {
    ...
    Pair<Boolean, byte[]> r1 =
        PrecompiledContracts.getContractForAddress(MLDSA_ADDR).execute(oneByteShort);
  ...
  public void trailingBytes_returnsZero() {
    ...
    Pair<Boolean, byte[]> result =
        PrecompiledContracts.getContractForAddress(MLDSA_ADDR).execute(padded);
